...
function [p,y,h,l]=subnp(p0,op,yy,ob,pb,h,l,ncon)
%
...
if nc>0.5,
  ...
  for i=1:np,
    ...
    ob=cost(p0(nic+1:npic).*scale(nc+2:nc+np+1),0,ncon)./scale(1:nc+1);
  ...
  %a is the gap between objective and constraint value
  %made modification on 11/28
  %[M,N]=size(a);
  %templambda=ones(M,N)*1e-8;
  %tempa=templambda+a;
  if cond(a)>1/eps,
    disp(msg);
  end
  %clear M N templambda tempa
  %try to handle NaN problem and Inf
  b=a*p0-constraint;
end
%
...
y=0; 
if ch>0,
  ob=cost(p(nic+1:npic).*scale(nc+2:nc+np+1),0,ncon)./scale(1:nc+1);
end
j=ob(1);
%
if nic>0.5,
...
while minit<maxit,
  ...
  if ch>0,
    for i=1:np,
      ...
      obm=cost(p(nic+1:npic).*scale(nc+2:nc+np+1),0,ncon)./scale(1:nc+1);
      if nic>0,
  ...
  ob3=cost(pt(nic+1:npic,3).*scale(nc+2:nc+np+1),-minit,ncon)./scale(1:nc+1);
  sob(3)=ob3(1);
  if nic>0.5,
  ...
  while go>tol,
    ...
    ob2=cost(pt(nic+1:npic,2).*scale(nc+2:nc+np+1),0,ncon)./scale(1:nc+1);
    sob(2)=ob2(1);
    if nic>0.5,
