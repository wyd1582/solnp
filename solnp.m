...
function [p,jh,l,h,ic,iteration]=solnp(pb,ib,op,l,h,ncon)
%
if exist('pb')<=0.5,
  disp('Syntax error')
  return
...
if exist('ib')<=0.5,...
  ...
  if nic>=0.5,
    ...
    %inequality and initial parameter value
    p=[ib0; p];
  end;
end;
clear ib ib0
...
%
opd=[1 1000 10 1.0e-5 1.0e-4];  % default optimization parameters
%
if exist('op')>=0.5,
  [m,n]=size(op);
  if m>1,
...
clear op opd
%np=number of parameters, chose the parameter with constraints
%should add more parameter including number of constraints
%only calculated the value of f(x)+constraint
%conflict between equality and inequality/linear and nonlinear
%test cost function-----failed 9:24AM
%try input required prob inputs-------10:08 AM
%--------------------
%amplcost(1)=AMPL.objType
%amplcost(2)=mLin
%amplcost(3)=mNonLin
%amplcost(4)=AMPL.sparseFlag
%--------------------
%amplcost=zeros(1,3);
%amplcost(1)=prob.AMPL.objType; 
%amplcost(2)=prob.mLin; 
%amplcost(2)=prob.mNonLin;
...
%ob=cost(p(nic+1:nic+np), 0,amplcost);
%ob1=cost(p(nic+1:nic+np), 0, amplcost);
%--------------------
ob=cost(p(nic+1:nic+np), 0,ncon);
[m,n]=size(ob);
if n>1.5,
  disp([om(1,:) 'Cost function must return a column vector'])
  return
end;
%m=row of objective vector, nic=number of inequality constraint
if m<nic+1,
  disp([om(3,:) 'The number of constraints in your COST function does';...
...
while iteration<maxit,
  ...
  %solving the lagrangian linearly constrained optimization problem
  [p,l,h,mu]=subnp(p,op,l,ob,pb,h,mu,ncon);
  disp('Updated parameters:')
  p(nic+1:nic+np),
   %change cost using amplfunc(x,0);
  ob=cost(p(nic+1:nic+np),iteration,ncon);
